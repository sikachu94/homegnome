...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      let p, c, e, g;
      try { const r = await window.storage.get("plantings"); p = r ? JSON.parse(r.value) : SEED_PLANTINGS; } catch { p = SEED_PLANTINGS; }
      try { const r = await window.storage.get("containers"); c = r ? JSON.parse(r.value) : SEED_CONTAINERS; } catch { c = SEED_CONTAINERS; }
      try { const r = await window.storage.get("events"); e = r ? JSON.parse(r.value) : SEED_EVENTS; } catch { e = SEED_EVENTS; }
      try {
        const r = await window.storage.get("garden");
        g = r ? JSON.parse(r.value) : null;
      } catch { g = null; }
      if (!g) {
        g = { ...DEFAULT_GARDEN, established_at: new Date().toISOString() };
        try { await window.storage.set("garden", JSON.stringify(g)); } catch (err) { console.error(err); }
      }
  ...
  const persist = useCallback(async (nextPlantings, nextEvents, nextContainers) => {
    try {
      if (nextPlantings) await window.storage.set("plantings", JSON.stringify(nextPlantings));
      if (nextEvents) await window.storage.set("events", JSON.stringify(nextEvents));
      if (nextContainers) await window.storage.set("containers", JSON.stringify(nextContainers));
  ...
  // ---- capture: note -> structured events ----------------------------------
  const runExtraction = async () => {
    if (!note.trim()) return;
    setExtracting(true); setExtractError(null);
    try {
      const plantingList = plantings.map((p) => `${p.id} | ${p.nickname} | ${p.species}`).join("\n");
      const typeList = Object.entries(EVENT_TYPES).map(([k, v]) => `- ${k} (${v.category}${v.scope === "garden" ? ", garden-level" : ""}): { ${v.fields} }`).join("\n");
      const system = `You convert a home gardener's freeform note (often a voice transcript) into structured event-log entries for homeGnome, a garden tracker.

Known plantings (id | nickname | species):
${plantingList}

Allowed event_type values, their category, and expected payload fields:
${typeList}
Phenophase values for stage_change: ${PHENOPHASES.join(", ")}

Respond with ONLY a JSON array, no prose, no markdown fences. Each item:
{"planting_id": "<id from the known plantings list, or null>", "event_type": "<one of the allowed types>", "category": "<matching category>", "payload": {...matching the fields for that type...}, "note": "<short human-readable note for anything not captured structurally>"}

For garden-level types (rainfall, frost) set "planting_id" to null. Never propose photo_log from text alone — photos are attached by the user directly, not inferred.
If the note describes more than one distinct thing, return multiple items. If nothing matches a known planting or event_type, return [].`;
      const raw = await askClaude(system, [{ role: "user", content: note }]);
      const parsed = JSON.parse(stripFences(raw));
      const withIds = (Array.isArray(parsed) ? parsed : []).map((d) => ({ ...d, draft_id: uid("draft"), media: [] }));
      setDrafts(withIds);
    } catch (err) {
      setExtractError("Couldn't parse that note — try rephrasing, or check your connection.");
  ...
  const saveDraft = async (draft) => {
    ...
    const next = [...events, buildEvent(draft)];
    setEvents(next); await persist(null, next); discardDraft(draft.draft_id);
  };
  const saveAllDrafts = async () => {
    const ready = drafts.filter((d) => scopeOf(d.event_type) === "garden" || d.planting_id);
    if (!ready.length) return;
    const next = [...events, ...ready.map(buildEvent)];
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      const recentEvents = [...events].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)).slice(0, 25)
        .map((e) => `${fmtDateTime(e.timestamp)} · ${labelForEntity(e)} · ${e.event_type} · ${JSON.stringify(e.payload)}${e.note ? ` · note: ${e.note}` : ""}`).join("\n");

      const system = `You are homeGnome, a home gardening assistant embedded in a garden tracker app. Answer using the gardener's own data below — be concise, warm, and practical. If the log doesn't have enough information to answer confidently, say so plainly instead of guessing.
...
${recentEvents}`;
      ...
      setChatMessages((ms) => [...ms, { role: "assistant", text: reply }]);
    } catch (err) {
  ...
  const resetDemo = async () => {
    ...
    setPlantings(SEED_PLANTINGS); setContainers(SEED_CONTAINERS); setEvents(SEED_EVENTS);
    setDrafts([]); setChatMessages([]); setGarden(freshGarden); setWeather(null);
  ...
  return (
    <div className="sg-root">
      ...
      <nav className="sg-tabs">
        ...
        <button className={tab === "chat" ? "active" : ""} onClick={() => setTab("chat")}><MessageCircle size={16} /> Ask homeGnome</button>
      </nav>
